...
public class RestaurantAnalyticsService : IRestaurantAnalyticsService
{
    ...
    {
        var revenue = await _uow.Analytics.GetRevenueSummaryAsync(filter, ct);

        var prevRevenue = await _uow.Analytics.GetRevenueSummaryAsync(prevFilter, ct);

        var orders = await _uow.Analytics.GetOrderSummaryAsync(filter, ct);

        var topDishes = await _uow.Analytics.GetTopDishesByRevenueAsync(filter, topN, ct);

        var topCategories = await _uow.Analytics.GetTopCategoriesAsync(filter, topN, ct);

        var payments = await _uow.Analytics.GetPaymentSummaryAsync(filter, ct);

        var retention = await _uow.Analytics.GetCustomerRetentionAsync(filter, ct);
