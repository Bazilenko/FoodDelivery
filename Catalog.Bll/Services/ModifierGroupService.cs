...
namespace Catalog.Bll.Services
{
    ...
    public class ModifierGroupService : IModifierGroupService
    {
        ...
        public ModifierGroupService(IUnitOfWork uow, IMapper mapper, IRestaurantContext restaurantContext)
        {
            _uow = uow;
            _mapper = mapper;
        ...
        {
            await AssertDishOwnershipAsync(dishId, ct);
        ...
        public async Task<ModifierGroupDto> GetByIdAsync(int id, CancellationToken ct = default)
            => _mapper.Map<ModifierGroupDto>(await GetOwnedAsync(id, ct));
        ...
        private async Task AssertDishOwnershipAsync(int dishId, CancellationToken ct)
        {
        ...
        private async Task<ModifierGroup> GetOwnedAsync(int id, CancellationToken ct)
        {
